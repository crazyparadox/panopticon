...
final class TimelapseStorageManager {
  ...
  private let queue = DispatchQueue(label: "com.panopticon.timelapse.purge", qos: .utility)

  private init() {
    let appSupport = fileMgr.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
    let path = appSupport.appendingPathComponent("Panopticon/timelapses", isDirectory: true)
    root = path
