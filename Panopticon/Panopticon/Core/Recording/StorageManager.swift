...
final class StorageManager: StorageManaging, @unchecked Sendable {
  ...
  let dbWriteQueue = DispatchQueue(label: "com.panopticon.storage.writes", qos: .utility)
  let dbContentionTracker = DatabaseContentionTracker()

  let purgeQ = DispatchQueue(label: "com.panopticon.storage.purge", qos: .background)
