...
enum TimelineFailureKind: String {
  ...
  case panopticonProRequired = "panopticon_pro_required"
...
struct TimelineFailureClassification {
  ...
  func toastContent(fallbackProviderLabel: String?) -> TimelineFailureToastContent? {
    ...
    switch kind {
    case .panopticonProRequired:
      return TimelineFailureToastContent(
        title: "Panopticon Pro required",
        body:
    ...
    case .cliNotInstalled:
      ...
      if let command = reauthCommand(for: provider) {
        body =
          "Panopticon talks to \(provider) through its CLI but couldn't find it. Reinstall it and run '\(command)' in Terminal, or switch providers in settings."
      } else {
        body =
          "Panopticon couldn't find the command-line tool for \(provider). Reinstall it, or switch providers in settings."
      }
      return TimelineFailureToastContent(
    ...
    case .localEngineOffline:
      return TimelineFailureToastContent(
        ...
        body:
          "Make sure it's running — Panopticon resumes with the next batch. Prefer zero setup? Gemini is free in provider settings.",
        destination: .providers
      )
    ...
    case .modelMisconfigured:
      return TimelineFailureToastContent(
        ...
        body:
          "The model Panopticon tried to use isn't available on \(provider). Check the model selection in provider settings, or switch providers.",
        destination: .providers
      )
  ...
  private func displayName(forProviderLabel label: String?) -> String? {
    ...
    case "panopticon": return "Panopticon"
...
enum TimelineFailureClassifier {
  ...
  private static let rules: [(kind: TimelineFailureKind, patterns: [String])] = [
    (.panopticonProRequired, ["panopticon pro is required"]),
    (
