...
final class LLMService: LLMServicing {
  ...
  private static let timelineFailureToastThrottleQueue = DispatchQueue(
    label: "com.panopticon.timelineFailureToastThrottle"
  )
  ...
  private func buildFailureToastMessage(
    ...
    if rateLimited && !backupConfigured {
      return
        "Panopticon hit a rate limit and no backup provider is configured. Add a backup in Settings > Providers to avoid interruptions."
    }

    switch operation {
    case .transcribing:
      return
        "Panopticon couldn't transcribe this batch. Check Settings > Providers and configure a backup provider."
    case .generatingCards:
      return
        "Panopticon couldn't generate timeline cards for this batch. Check Settings > Providers and configure a backup provider."
    case .none:
      return
        "Panopticon couldn't finish this batch. Check Settings > Providers and configure a backup provider."
    }
  }
