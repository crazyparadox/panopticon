...
struct SettingsProvidersTabView: View {
  ...
  private var geminiModelSection: some View {
    SettingsSection(
      ...
      subtitle: "Choose which Gemini model Panopticon should prioritize."
    ) {
      VStack(alignment: .leading, spacing: 14) {
  ...
  private var promptCustomizationSection: some View {
    ...
    case "gemini":
      promptSection(
        ...
        subtitle: "Override Panopticon's defaults to tailor card generation.",
        intro:
          "Overrides apply only when their toggle is on. Unchecked sections fall back to Panopticon's defaults.",
        sections: [
          promptEditorConfig(
            heading: "Card titles",
    ...
    SettingsSection(title: title, subtitle: subtitle) {
      VStack(alignment: .leading, spacing: 18) {
        ...
        HStack {
          Spacer()
          SettingsSecondaryButton(
            title: "Reset to Panopticon defaults",
