...
let timelinePerfLogger = Logger(
  subsystem: Bundle.main.bundleIdentifier ?? "teleportlabs.com.Panopticon",
  category: "TimelinePerf"
)

#if DEBUG
  private let timelinePerfLogFileURL = URL(fileURLWithPath: "/tmp/panopticon-timeline-perf.log")
  private let timelinePerfLogQueue = DispatchQueue(
    label: "teleportlabs.com.Panopticon.timelinePerfLog"
  )
...
/// The two surfaces the shell can show. The Dayflow sidebar is gone; a gear
/// button in the top-right toggles between them.
enum SidebarIcon {
  case timeline
  case settings
}
...
extension MainView {
  ...
  func syncCurrentUIContext(
    ...
    // Analytics UI-context tracking removed; kept as a no-op so call sites
